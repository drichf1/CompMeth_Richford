...
def calculate_binding_energy(atomic_number, mass_number):
    ...
    if (mass_number == 0): #making sure we don't divide by zero
        return energy_array
    else:
...
mass_number_a = atomic_number_z-1 #index for while loop -- starts at only-proton nucleus
...
for mass_number_a in range(atomic_number_z-1, 3*atomic_number_z+1): # loop through range A=Z to A=3Z
    ...
    #print("Z = ",atomic_number_z," A = ",mass_number_a,"Binding Energy per Nucleon: ",energy_array_new[1])
...
print("(d)")
atomic_number_z = -1 #index for the loop (starting at negative 1 so as to account for Z=0)
mass_number_a = -1 #index for the loop
...
for atomic_number_z in range(-1,118):
    ...
    for mass_number_a in range(atomic_number_z-1,3*atomic_number_z+1):
        mass_number_a += 1
        energy_array_new = calculate_binding_energy(atomic_number_z, mass_number_a)
        #print("Z = ",atomic_number_z,", A = ",mass_number_a,"; Binding Energy = ", energy_array_new[0], "MeV, Binding Energy per Nucleon: ",energy_array_new[1], " MeV")
        if (atomic_number_z == 0 and mass_number_a == 1): # seperately account for neutron alone
            energy_array_old = energy_array_new
            mass_number_store_a = mass_number_a
        elif (energy_array_old[1] < energy_array_new[1]): #if new binding energy is greater than the maximum we recorded: swap in the new values
            energy_array_old = energy_array_new
            atomic_number_store = atomic_number_z
            mass_number_store = mass_number_a
            #print("    Swap! Record new Binding Energy per Nucleon: ",energy_array_old[1])
